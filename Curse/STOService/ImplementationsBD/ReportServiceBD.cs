...
namespace STOService.ImplementationsBD
{
    public class ReportServiceBD : IReportService
    {
        ...
        public List<ClientOrdersModel> GetClientOrders()
        {
            ReportBindingModel model = new ReportBindingModel {
                DateFrom = MainServiceBD.dateFrom,
                DateTo = MainServiceBD.dateTo
            };
            Queue<String> servs = new Queue<string>();
            Queue<String> ress = new Queue<string>();

            List<ClientOrdersModel> com = context.Orders
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo && rec.client.id == MainServiceBD.iDClient)
                            .Select(rec => new ClientOrdersModel
                            {
                                serviceList = rec.orderServices.Select(os => new ServiceViewModel
                                {
            ...
            int idClient = MainServiceBD.iDClient;
            com.Clear();
            com = context.Orders
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo && rec.client.id == MainServiceBD.iDClient)
                            .Select(rec => new ClientOrdersModel
            ...
            foreach (var o in com)
            {
                ...
                o.serviceListStr = servs.Count > 0 ? servs.Dequeue() : "";
                o.resourceListStr = ress.Count > 0 ? ress.Dequeue() : "";
            }
            return com;
        }
        ...
        public void SaveClientOrders(ReportBindingModel model)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);cell = new PdfPCell(new Phrase(list[i].status, fontForCells));
                table.AddCell(cell);
